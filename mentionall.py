...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Əzizim Bu əmr yalnız qruplar üçün etibarlıdır!**")
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("**😌 Tag'a başlamaq üçün bir səbəb yazın...!**")
...
async def mentionall(event):
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Şirinim Bu əmr yalnız qruplar üçün etibarlıdır!** ")
  ...
  if event.pattern_match.group(1):
    ...
    return await event.respond("**😜 Başlamaq üçün heç bir səbəb yoxdur, Səbəb yazın...! **")
...
async def mentionall(event):
  ...
  if event.is_private:
    return await event.respond("**Urəm Bu əmr yalnız qruplar üçün etibarlıdır!**")
...
"Sevgisinideyən birini yox, go‌stərənbirini sevin..🤍🖇️",
"• ᴅüɴʏᴀɴıɴ ᴇɴ ɢüçʟü ɪɴsᴀɴı ʙɪʟᴇ ᴏʟsᴀɴ, sᴇᴠᴅɪğɪɴᴇ ᴋᴀʀşı ᴋᴀʏʙᴇᴅᴇʀsɪɴ🫀",
"ʜəʀ ᴋəsə ɢöᴢüᴍʟə ʙᴀхᴅıᴍ, səɴə ɪsə üʀəʏɪᴍʟə🔗🤍",
"Hər kəs sənin kimi gülüb danışa bilər. Amma heç kim sən ola bilməz.⚡️🤍",
...
"Dost sözü hecaya bölünməzdir🥹❤️Hərkəsin pis günündə yanında olacaq dostu olması diləyiylə..🌸💙",
"Hansı yaxşılığımın əvəzisən sən?)...🤍",
"Sevgi insanı həyata bağlar, Həyat isə insanı sevgiyə bağlar..🤍🥂",
...
async def mentionall(event):
  ...
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Həyatım Bu əmr yalnız qruplar üçün etibarlıdır!** ")
...
async def mentionall(event):
  ...
  global anlik_calisan
  if event.is_private:
    return await event.respond("**Beybi Bu əmr yalnız qruplar üçün etibarlıdır!** ")
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      usrnum += 1
      usrtxt += f"↯ [{random.choice(sema)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("**Proses Uğurla Dayandırıldı ⌛**")
        return
      if usrnum == 1: 
